...
window.draw_deptree = (sent, sent_id, hover_fun=->) ->
...
webFontURLs = [
    window.deptree_lib_prefix+"/brat/static/fonts/Astloch-Bold.ttf"
    window.deptree_lib_prefix+"/brat/static/fonts/PT_Sans-Caption-Web-Regular.ttf"
    window.deptree_lib_prefix+"/brat/static/fonts/Liberation_Sans-Regular.ttf"
]
...
make_entity_from_pos = (p) ->
    type: p
    labels: [p]
    bgColor: color_from_chars(p, 0.8, 0.95, 0.75)
    borderColor: "darken"
...
draw_brat_tree = (words, to_div, hover_fun) ->
    ...
    if (!dispatcher)
    	...
    	new Visualizer(dispatcher, to_div, webFontURLs);
    ...
    head.ready ->
        ...
        dispatcher.on 'doneRendering', ->
            ...
            _.map div.find("g.span text"), (g) ->
                pos = $(g).text()
                $(g).parent().hover -> hover_fun pos: pos, data: refMap[$(g.parentNode).find("rect").data("span-id")]
